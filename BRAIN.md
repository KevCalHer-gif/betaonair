...
**ANÁLISIS DE ENLACES (páginas programa, noticias, etc.):**

- **/programas** – Debería listar programas desde CMS, los enlaces de cada programa a su slug (`/programas/[slug]`) están implementados en ProgramCard. Funcionalidad depende de que existan datos en Payload. **Potencial problema**: Ningún dato de prueba → tarjeta visible pero sin contenido.

- **/programas/[slug]** (detalle) – Página planeada pero no confirmada si existe en código. BRAIN.md menciona que debe usar `getProgram` y `getEpisodes`, pero no se ha confirmado implementación. **Enlace puede no funcionar**.

- **/noticias** – No aparece en sumarios. Posiblemente redirige. La página de listado de noticias no está desarrollada completamente según backlog. **Enlace roto o redirección**.

- **/noticias/[slug]** – Se definió como Server Component con `params: Promise`. Parece implementado (según commit). **Funcional si hay datos**.

- **/en‑vivo** – Página según sumario utiliza `getLiveStreams`. Puede mostrar embed correctamente si hay un stream activo. **Depende de datos**; podría mostrar estado vacío.

- **/contacto** – Formulario existe pero la colección `contacts` requiere autenticación para crear mensajes, por lo que el envío fallará para visitantes no logueados. **Enlace (acción) no funcional**. Se debe cambiar `access.create` a `() => true`.

- **/portafolio** – Actualmente redirige a `/programas`. **No es una página real**. Pendiente definir fuente de datos.

- **/servicios** – No implementada según backlog (propósito futuro). **Sin enlace**.

- **Navegación global** – Depende del layout global que aún no existe (está en backlog). **Enlaces del menú pueden estar ausentes**.

**RIESGOS DETECTADOS:**
- Contacto no puede enviar datos (riesgo crítico).
- Portafolio solo redirige (confunde usuario).
- Sin datos de prueba la mayoría de páginas quedan vacías.
- Layout global pendiente, por lo que no hay navegación consistente entre páginas.

**RECOMENDACIÓN:** Priorizar arreglo de acceso de colección `contacts` e implementación de layout global antes de continuar con otras páginas.
